...
- (void)useMethod{
    ...
    NSLog(@"字符串拼接 = %@",[NSString getABwithA:aa andB:bb]);
    ...
    NSString* strAA = @"girlds";
    NSLog(@" isContainStringA = %d",[NSString stringA:strAA isContainStringB:string]);
     ...
     */
    //NSString *urlStr = [NSString stringWithContentsOfURL:[NSURL URLWithString:@"http://baidu.com"]];
    NSURL *URL = [NSURL URLWithString:@"http://baidu.com"];
    //1 NSURL转换成NSString
    NSString * urlString = [URL absoluteString];   //url>string
    
    //2 NSString转换成NSURL
    NSURL * url = [NSURL URLWithString:urlString]; //string>url
     ...
     *  从某个字符开始，截取到最后一位
     */
    NSString *urlStr = @"http://172.16.1.150:8080/HQ1.0.0/index.jsp?code=600346";
    NSString* resultStr = [NSString cutString:urlStr startString:@"=" isContain:NO];
    NSLog(@"---> resultStr = %@",resultStr); //结果是：600346
...
// 字符串拼接
+ (NSString *)getABwithA:(NSString *)A andB:(NSString *)B
{
...
+ (NSString *)cutString:(NSString *)info startString:(NSString *)startStrA endString:(NSString *)endStrB isContain:(BOOL)isContain {
    ...
    if ( [info rangeOfString:startStrA].location == NSNotFound ) {
        resultString = @"目的字符串_找不到_开始截取的字符";
        
    }else if ([info rangeOfString:endStrB].location == NSNotFound ){
        resultString = @"目的字符串_找不到_结束截取的字符";
        
    }else if ( ([info rangeOfString:startStrA].location == NSNotFound) && [info rangeOfString:endStrB].location == NSNotFound) {
        resultString = @"目的字符串_找不到_开始和结束的字符";
    }else{
        resultString = [info substringWithRange:cutRange];  //(共截取的范围)
    }
    return resultString;
}

/**
 *  截取字符串：从检测到的第一个字符开始截取 到最后一位（可能会有多个相同的字符）
 *
 *  @param info         目的字符串
 *  @param startStrA    开始字符 (不是字符串)
 *  @param isContain    是否包含：开始字符
 *
 *  @return 截取的字符
 */
+ (NSString *)cutString:(NSString *)info
            startString:(NSString *)startStrA
              isContain:(BOOL)isContain {
    ...
    NSRange cutRange;
    if (isContain) {
        cutRange = NSMakeRange(textRang1.location, info.length-textRang1.location-1);
    }else{
        cutRange = NSMakeRange(textRang1.location+1, info.length-textRang1.location-1);
    }
    NSString* resultString = nil;
    if ( [info rangeOfString:startStrA].location == NSNotFound ) {
        resultString = @"目的字符串_找不到_开始截取的字符";
